...
@section('content')
                ...
                <div class="btn-list">
                    <form action="{{ route('discountcodes.create') }}" method="POST">
                        ...
                        <button type="submit" class="btn btn-primary d-none d-sm-inline-block" data-bs-toggle="modal" data-bs-target="#modal-report">
                            ...
                            Crear nuevo codigo de descuento 
                        </button>
                    </form>
                </div>
            </div>
    ...
    <form class="d-flex flex-row form-inline py-2 col-5 gap-2 mb-4" action="{{ route('discountcodes.index') }}" method="GET">
    ...
    <div class="table-responsive">
        <table class="table table-vcenter">
            ...
            @foreach ($discountcodes as $discountcode)
                <tr>
                    ...
                    <td>{{ $discountcode->status == 'active' ? "Activo" : "Inactivo" }}</td>
        ...
        <nav aria-label="Page navigation example" class="mt-4">
            <ul class="pagination gap-2">
                <li class="btn btn-dark btn-sm"><a class="text-white" href="{{ route('discountcodes.index', ['page' => ($discountcodes->currentPage()-1)]) }}">Anterior</a></li>
                @for ($i = 1; $i <= $discountcodes->lastPage(); $i++)
                    <li class="btn btn-dark btn-sm bg-dark {{ ($i == $discountcodes->currentPage()) ? ' active' : '' }}"><a class="page-link bg-dark" href="{{ route('discountcodes.index', ['page' => $i]) }}">{{ $i }}</a></li>
                @endfor
                <li class="btn btn-dark btn-sm"><a class="text-white" href="{{ route('discountcodes.index', ['page' => ($discountcodes->currentPage()+1)]) }}">Siguiente</a></li>
            </ul>
        </nav>
    </div>
    </div>
