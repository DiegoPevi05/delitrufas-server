...
@section('content')
    ...
    <form method="POST" action="{{ route('discountcodes.update', $discountcode) }}" class="col-6">
        ...
        <div class="form-group my-2">
            ...
            <input type="date" class="form-control @error('expired_date') is-invalid @enderror" id="expired_date" name="expired_date" value="{{ old('expired_date', $discountcode->expired_date) }}">
            @error('expired_date')
            <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
