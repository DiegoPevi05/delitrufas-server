...
@section('content')
    ...
    <form method="POST" action="{{ route('specialiststimes.update', $specialiststime)}}" class="col-6">
        @csrf
        @method('PUT')
        <div class="form-group my-2">
            ...
            <input type="hidden" id="specialist_id" name="specialist_id" value="{{ old('specialist_id', $specialiststime->specialist_id) }}">
            <div class="row g-2">
                <div class="col">
                    <input type="text" class="form-control @error('specialist_id') is-invalid @enderror" id="specialist_name" name="specialist_name" placeholder="Buscar Especialista" value="{{ old('specialist_name', $specialiststime->specialist->user->name) }}" >
                </div>
                <div class="col-auto">
        ...
        <div class="form-group my-2">
            ...
            <input type="datetime-local" class="form-control @error('start_date') is-invalid @enderror" id="start_date" name="start_date" value={{old('start_date', $specialiststime->start_date)}}>
            @error('start_date')
            <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        <div class="form-group my-2">
            ...
            <input type="datetime-local" class="form-control @error('end_date') is-invalid @enderror" id="end_date" name="end_date" value={{old('end_date', $specialiststime->end_date)}}>
            @error('end_date')
            <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
    ...
    @else
        <div class="row mt-3">
            <div class="row-5">
                ...
                <a href={{ route('specialiststimes.index') }} class="btn btn-primary">Voler a la lista de Horas</a>
            </div>
        </div>
    @endif
@endsection
